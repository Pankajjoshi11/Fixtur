...
export default function MatchDetailView({ matchId, onBackToLobby }: MatchDetailViewProps) {
  ...
  useEffect(() => {
    ...
    // 1. FETCH FULL STATE: Grab delivery history & full Zustand state from backend
    const fetchCurrentMatchState = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.error("Failed to fetch initial match state:", res.statusText);
        }
      } catch (error) {
    ...
    // 2. PUSHER: Subscribe for live ball-by-ball updates
    ...
    matchChannel.bind('score-update', (serverState: any) => {
      ...
      if (serverState.bowlingTeam) setBattingTeam(serverState.bowlingTeam);
    });
  ...
  return (
    <main className="max-w-3xl mx-auto space-y-6">
      <button 
        ...
        className="text-emerald-500 hover:text-emerald-400 text-sm font-semibold flex items-center gap-1 transition-colors mb-4"
      >
